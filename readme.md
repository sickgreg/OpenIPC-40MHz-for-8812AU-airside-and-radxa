...
If anyone wants to try 40MHz channels they can replace their /usr/bin/wifibroadcast script on their openIPC camera with this one (rename it to `wifibroadcast` after copying it to cam), which will set the adapter to 40MHz upon loading the driver.  It's a bit of a hack.  Just restore the old file to go back.

Also update /etc/wfb.conf to bandwidth=40

(btw, I have only tried channel 161, hence the hardcoded ch 161 in the following script. Feel free to change it)
